...
describe("endpoints", () => {
  describe("internal polygon url", () => {
    it("prefers wallet from query", () => {
      const mockQuery = {
        wallet: "foobar",
        account: "baz",
      };
      const expected = "/api/polygon?wallet_address=foobar";
      const actual = polygonWalletURI(mockQuery);
      expect(actual).toBe(expected);
    });
    it("selects address if no wallet is present", () => {
      const mockQuery = {
        account: "baz",
      };
      const expected = "/api/polygon?wallet_address=baz";
      const actual = polygonWalletURI(mockQuery);
      expect(actual).toBe(expected);
    });
    it("adds debug flag when enabled", () => {
      const mockQuery = {
        account: "baz",
        debug: 111111111,
      };
      const expected = "/api/polygon?wallet_address=baz&debug=111111111";
      const actual = polygonWalletURI(mockQuery);
      expect(actual).toBe(expected);
    });
  });
